...
const getSupabaseConfig = () => {
  ...
  const key = process.env.SUPABASE_LEADS_INSERT_KEY || process.env.SUPABASE_SERVICE_ROLE_KEY || "";
  const table = process.env.SUPABASE_LEADS_TABLE || "leads";

  if (!url || !key) {
    throw new LeadStorageError("Configuracao do Supabase ausente.", {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new LeadStorageError("Falha ao salvar lead.", {
        ...
        details: `${response.status} ${response.statusText || ""} ${responseText}`.trim().slice(0, 500),
      });
    }
  } finally {

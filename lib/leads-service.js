...
const LEAD_TYPES = new Set(["Pessoa fisica", "Empresa / frota"]);
...
const validateLead = (lead) => {
  ...
  if (lead.tipo && !LEAD_TYPES.has(lead.tipo)) {
    errors.push("tipo");
  }
...
const getSupabaseConfig = () => {
  ...
  if (!url || !key) {
    throw new Error("Configure SUPABASE_URL e SUPABASE_SERVICE_ROLE_KEY para salvar os leads.");
  }
...
const saveLeadToSupabase = async (lead) => {
  if (typeof fetch !== "function") {
    throw new Error("A runtime atual nao oferece suporte a fetch.");
  }

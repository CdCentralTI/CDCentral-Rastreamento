...
<<<<<<< HEAD
const shouldUseMemoryRateLimitFallback = () => process.env.NODE_ENV !== "production";
=======
...
<<<<<<< HEAD
const createRateLimiter = ({ windowMs, maxRequests, keyPrefix = "", maxKeys = 5000 }) => {
=======
...
>>>>>>> 5b8dd71 (mundando para o node.js)
  ...
  return async (key) => {
    ...
    if (!redisScript) {
<<<<<<< HEAD
      if (!shouldUseMemoryRateLimitFallback()) {
        throw new HttpError(500, "Rate limiter indisponivel.", "missing_rate_limiter_config");
=======
      if (
        !shouldUseMemoryRateLimitFallback({
